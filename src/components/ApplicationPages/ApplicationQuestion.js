...
export default function ApplicationQuestion() {
  ...
  async function onSubmit(data) {
    debugger
    var allTickets =  await database.collection('tickets').orderBy('timestamp', 'desc').get();
    var selectedTicket =  allTickets.docs.map((doc) => ({ id: doc.id, data: doc.data()  })).find( t => t.id == data.FormQuestion);
    if(!selectedTicket){
      // alert("404"); 
        ...
        >
          <div className="form-row">
            <div className="form-group col-5">
              <label className="text-gray-600 font-medium">Başvuru Sorgula</label>
              <input
                name="FormQuestion"
                type="text"
